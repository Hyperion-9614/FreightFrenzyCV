...
public class DuckPipeline extends OpenCvPipeline {
    ...
    public Mat processFrame(Mat inputMat) {
        ...
        Imgproc.findContours(inputMat, points, hierarchy, Imgproc.RETR_LIST,
        ...
        Imgproc.rectangle(outputMat, boundingRect.tl(), boundingRect.br(), new Scalar(0, 255, 0),
                2);
        ...
        Imgproc.line(outputMat, new Point(outputMat.width() / 3, 0),
                new Point(outputMat.width() / 3, outputMat.height()), new Scalar(255, 0, 255), 2);

        //draw a bright pink line for the third third of the frame that runs from top to bottom
        Imgproc.line(outputMat, new Point(outputMat.width() * 2 / 3, 0),
                new Point(outputMat.width() * 2 / 3, outputMat.height()), new Scalar(255, 0, 255),
                2);
        ...
        if (centerX < outputMat.width() / 3) {
            Imgproc.putText(outputMat, "1", new Point(boundingRect.x, boundingRect.y - 10),
                    Imgproc.FONT_HERSHEY_SIMPLEX, 1, new Scalar(0, 255, 255), 2);
            position = 1;
        } else if (centerX < outputMat.width() * 2 / 3) {
            Imgproc.putText(outputMat, "2", new Point(boundingRect.x, boundingRect.y - 10),
                    Imgproc.FONT_HERSHEY_SIMPLEX, 1, new Scalar(0, 255, 255), 2);
            position = 2;
        } else {
            Imgproc.putText(outputMat, "3", new Point(boundingRect.x, boundingRect.y - 10),
                    Imgproc.FONT_HERSHEY_SIMPLEX, 1, new Scalar(0, 255, 255), 2);
            position = 3;
        }
