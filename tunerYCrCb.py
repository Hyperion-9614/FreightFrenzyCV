...
class ColorTuner:
    ...
    def main(self, path, downScale):
        ...
        image = cv2.resize(unsizedImage,
                           (int(unsizedImage.shape[1] / downScale), (int(unsizedImage.shape[0] / downScale))))
        # Create a window
        ...
        while 1:
            ...
            if ((pYMin != YMin) | (pCrMin != CrMin) | (pCbMin != CbMin) | (pYMax != YMax) | (pCrMax != CrMax) | (
                    pCbMax != CbMax)):
                print("(YMin = %d , CrMin = %d, CbMin = %d), (YMax = %d , CrMax = %d, CbMax = %d)" % (
...
if __name__ == "__main__":
    colorTuner = ColorTuner('/Users/aditya/Programming/FTC/image.jpg', 1)